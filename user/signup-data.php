...
if (isset($_POST['btnSubmit'])) {
    ...
    $sql = "SELECT cid FROM tblCity WHERE pincode = :pincode && cname = :cname";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['cname' => $city, 'pincode' => $user['Pincode']]);
    // $stmt->execute(['area' => $user['Address2'], 'pincode' => $user['Pincode']]);
    $verifyCity = $stmt->fetch();
    ...
    $sql = "SELECT uid FROM tblUser WHERE contactnumber = :contactno";
    $stmt = $pdo->prepare($sql);
    ...
    // no same usernames
    $sql = "SELECT lid FROM tblLogin WHERE username = :username";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['username' => $user['Username']]);
    $verifyLogin = $stmt->fetch();
